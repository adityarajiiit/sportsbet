...
function UpcomingEventCard({
  ...
  const handleNotify = async () => {
    ...
    try {
      ...
      await axios.post(
        `/api-backend/api/reminders/newreminder`,
        {
  ...
  return (
    <div className="p-5 flex flex-col justify-center items-center bg-base-200 gap-2 rounded-xl border border-neutral-content/5 h-[14rem]">
      ...
      <div className="w-full flex justify-center items-center mt-2">
        <span className="font-poppins font-semibold text-xs line-clamp-2">
          {status}
        </span>
      </div>
      <button
      ...
      >
        <FaBell />{" "}
        {notifying ? "Setting..." : notified ? "Reminder Set" : "Notify Me"}
      </button>
      <div className="w-full flex justify-center items-center mt-2">
